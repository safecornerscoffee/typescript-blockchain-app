...
export class BlockchainNode {
  ...
  async calculateHash(block: WithoutHash<Block>): Promise<string> {
  ...
  async addBlock(newBlock: Block): Promise<void> {
    ...
    if (previousBlockIndex < 0) {
      throw new Error(`${errorMessagePrefix} - there is no block in the chain with the spectified previous hash "${newBlock.previousHash.substr(0, 8)}".`);
    }

    const tail = this._chain.slice(previousBlockIndex + 1);
    if (tail.length >= 1) {
      throw new Error(`${errorMessagePrefix} - the longer tail of the current node taks precedence over the new block.`);
    }
...
export const randomDelay = (maxMilliseconds: number = 100): Promise<void> => {
